...
class SyncCloudSigner(SyncEndpoint):
    ...
    def create_and_wait_session(
        ...
        *,
        ...
        session = self.create_session(duration=duration, client_id=client_id)
        auth_session_id = session.auth_session_id

        start = time.monotonic()
        while time.monotonic() - start < timeout:
            check = self.check_session(auth_session_id=auth_session_id)
        ...
        *,
        ...
        while time.monotonic() - start < timeout:
            if use_refresh_token:
                refresh_check = self.check_refresh_session(
                    auth_session_id=auth_session_id,
                )
