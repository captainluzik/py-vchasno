...
class SyncCloudSigner(SyncEndpoint):
    ...
    def create_session(self, *, duration: int, client_id: str, use_refresh_token: bool = False) -> CloudSignerSession:
        ...
        data = self._request("POST", "/api/v2/cloud-signer/sessions/create", json={
            "duration": duration, "client_id": client_id, "use_refresh_token": use_refresh_token,
        })
    ...
    def check_refresh_session(self, *, auth_session_id: str) -> CloudSignerRefreshCheck:
        ...
        data = self._request("POST", "/api/v2/cloud-signer/sessions/refresh/check", json={"auth_session_id": auth_session_id})
    ...
    def refresh_token(self, *, auth_session_id: str, refresh_token: str) -> CloudSignerRefresh:
        ...
        data = self._request("POST", "/api/v2/cloud-signer/sessions/refresh", json={
            "auth_session_id": auth_session_id, "refresh_token": refresh_token,
        })
        return CloudSignerRefresh.model_validate(data)

    def sign_document(self, *, client_id: str, password: str, document_id: str, auth_session_token: str | None = None, access_token: str | None = None) -> Any:
    ...
    def create_sign_session(self, *, document_id: str, edrpou: str, email: str, type: str, on_cancel_url: str | None = None, on_finish_url: str | None = None) -> SignSession:
...
class AsyncCloudSigner(AsyncEndpoint):
    ...
    async def create_session(self, *, duration: int, client_id: str, use_refresh_token: bool = False) -> CloudSignerSession:
        data = await self._request("POST", "/api/v2/cloud-signer/sessions/create", json={
            "duration": duration, "client_id": client_id, "use_refresh_token": use_refresh_token,
        })
        return CloudSignerSession.model_validate(data)

    async def check_session(self, *, auth_session_id: str) -> CloudSignerSessionCheck:
        data = await self._request("POST", "/api/v2/cloud-signer/sessions/check", json={"auth_session_id": auth_session_id})
        return CloudSignerSessionCheck.model_validate(data)

    async def check_refresh_session(self, *, auth_session_id: str) -> CloudSignerRefreshCheck:
        data = await self._request("POST", "/api/v2/cloud-signer/sessions/refresh/check", json={"auth_session_id": auth_session_id})
        return CloudSignerRefreshCheck.model_validate(data)

    async def refresh_token(self, *, auth_session_id: str, refresh_token: str) -> CloudSignerRefresh:
        data = await self._request("POST", "/api/v2/cloud-signer/sessions/refresh", json={
            "auth_session_id": auth_session_id, "refresh_token": refresh_token,
        })
        return CloudSignerRefresh.model_validate(data)

    async def sign_document(self, *, client_id: str, password: str, document_id: str, auth_session_token: str | None = None, access_token: str | None = None) -> Any:
        body: dict[str, Any] = {"client_id": client_id, "password": password, "document_id": document_id}
        if auth_session_token:
            body["auth_session_token"] = auth_session_token
        if access_token:
    ...
    async def create_sign_session(self, *, document_id: str, edrpou: str, email: str, type: str, on_cancel_url: str | None = None, on_finish_url: str | None = None) -> SignSession:
        body: dict[str, Any] = {"document_id": document_id, "edrpou": edrpou, "email": email, "type": type}
        if on_cancel_url:
            body["on_cancel_url"] = on_cancel_url
        if on_finish_url:
