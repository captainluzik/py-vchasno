...
class SyncArchive(SyncEndpoint):
    ...
    def upload_scans(self, files: list[str | Path | BinaryIO], *, parent_id: int | None = None) -> ArchiveScanResult:
        """POST /api/v2/archive/scans."""
        params: dict[str, Any] = {}
        if parent_id is not None:
            params["parent_id"] = parent_id
    ...
    def import_signed_external(
        ...
        **metadata: Any,
        ...
        """POST /api/v2/archive/import-signed (external format)."""
        opened: list[Any] = []
        ...
        for k, v in metadata.items():
            if v is not None:
                data_fields[k] = str(v)
        try:
            data = self._request("POST", "/api/v2/archive/import-signed", files=file_tuples, data=data_fields or None)
        finally:
            for f in opened:
                f.close()
    ...
    def import_signed_internal(
        ...
        **metadata: Any,
        ...
        """POST /api/v2/archive/import-signed (internal format)."""
        opened: list[Any] = []
...
class AsyncArchive(AsyncEndpoint):
    ...
    async def upload_scans(
        ...
        for f in files:
            if isinstance(f, (str, Path)):
                ...
                file_tuples.append(("files", ("scan", f)))
        try:
            data = await self._request("POST", "/api/v2/archive/scans", params=params or None, files=file_tuples)
        finally:
            for fp in opened:
                fp.close()
        return ArchiveScanResult.model_validate(data)

    async def import_signed_external(
        self,
        file: str | Path | BinaryIO,
        signatures: list[str | Path | BinaryIO],
        *,
    ...
    async def import_signed_internal(
        self,
        signed_file: str | Path | BinaryIO,
        *,
        filename: str | None = None,
